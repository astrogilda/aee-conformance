...
`research/236-geo-deep-2026-08-10/measure_sentence_coverage.py` in the
...
work proposed a NEW vector for row 4 and the mutation proof killed the proposal:
...
So the claim was attacked rather than reviewed. An anchor reading `L1725-1748`
...
by `bad-208` as well as `bad-201`, and L997 by `bad-714` as well as `bad-106`
...
**A negative index on a non-substrate row is admitted by both rails.** L920
...
`ok-054` forces L1588 and the measurement still counts that sentence as cited by
...
```
python3 research/236-geo-deep-2026-08-10/measure_sentence_coverage.py   # in the consuming repo
