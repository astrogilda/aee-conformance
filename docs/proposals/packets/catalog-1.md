...
| **deadline** | none stated. The issue's own rules of engagement say "No response in 21 days -> we treat that as 'no takers yet', not as evidence of strength." Verified by reading the issue body, captured 2026-09-09 at `research/366-corpora-anchor-acs-mcp-2026-09-09/_raw/github/aos-catalog-issue-1.json` |
| **audience** | The repository's maintainer, who wrote 15 of the 21 comments. Two other reporters have taken the invitation and both post reproduction receipts, 5 comments and 4 comments. Everything posted here will be re-run by at least one of them. Handles are in the internal execution report and are deliberately absent from this file, which sits in a public repository |
| **identity** | Private individual, the operator's own name. The operator's ruling of 2026-09-09 (D-036 item 2) overrides the no-link rule for this venue only: the corpus is linked and the company, sandbox and site stay unnamed. Every payload is still inline, so the findings reproduce without following the link |
...
3. The pull request in `catalog-pr.md` follows the comment, so the maintainer sees the findings before he sees a directory.

4. Told next: nobody outside.

5. Follow-up cadence: if he replies with a repair, re-run the same script against the new tag and post the table again, which is the exchange the two other reporters have already established in this thread.

6. If nobody replies in 21 days, his own rules of engagement have already said what that means, and no nudge is sent.
...
One question. The four repairs turn streams that return PARTIAL today into FAILED, so by your own convention they ship as a new tag and v0.10 stays where it is.
